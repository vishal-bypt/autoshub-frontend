        ...
        if (result.isConfirmed) {
          ...
          if (recoverId) {
            if (userDetails.role === "Admin") {
              rfcService
                .update(recoverId, { userId: userDetails?.id, isEdit: 3 })
                .then(() => {
                  rfcService.getAll("latest").then((x) => setUsers(x));
                  Swal.fire({
                    title: `Your RFP record recovered successfully`,
                  });
                })
                .catch((error) => {
      ...
      <div className="container-fluid">
        <div className="row">
          ...
          <div className="col-md-6 text-end">
            <Link to={`/rfp/add`} className="btn btn-primary">
              Add
            </Link>
            <Link to={`/rfp/landing-entries`} className="btn btn-primary ml-1">
              Landing Page
            </Link>
            <Link to={`/rfp/my-entries`} className="btn btn-primary ml-1">
              My Entries
            </Link>
            <Link
              to="#"
            ...
            <table className="table table-striped allEntries extra-black-table">
              <thead>
                <tr>
                  ...
                  {(userDetails.role === "Manager" ||
                    userDetails.role === "Admin") && (
                    <th className="traning-listing">Ageing Days</th>
                  )}
              ...
              <tbody>
                {users &&
                  users.map((user, index) => {
                    var ageingDate = moment(
                      user?.rfpRespondedDate,
                      "YYYY-MM-DD"
                    ).add(60, "days");
                    ...
                    return (
                      <tr key={index + "HomeList"}>
                        ...
                        <td
                          ...
                          style={{ minWidth: "150px" }}
                        >
                          {moment(user.monthCalendar, "YYYY-MM-DD").format(
                            "MM/YYYY"
                          )}
                        </td>
                        <td
